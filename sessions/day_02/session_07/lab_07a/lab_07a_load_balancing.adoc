...
This lab uses 4 different applications. Here is a table that you can refer to regarding the corresponding port numbers.
...
. Config Server: open `springtrader-config-server` and launch the class `SpringtraderConfigServerApplication`

. Connect to http://localhost:8888/demo/default to confirm that your config server has been started successfully. 
...
. Run the `consumer` application by launching `SpringtraderConsumerApplication` from your IDE


. Connect to http://localhost:8091 in order to test the consumer application. It should receive values from the producers.
